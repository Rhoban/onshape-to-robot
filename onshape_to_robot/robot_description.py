...
class RobotURDF(RobotDescription):
    ...
    def addDummyLink(self, name, visualMatrix=None, visualSTL=None, visualColor=None):
        ...
        if visualSTL is not None:
            self.addSTL(
                visualMatrix, visualSTL, visualColor, name + "_visual", "visual"
            )
        self.append("</link>")
    ...
    def endLink(self):
        ...
        self.append("<inertial>")
        self.append(
            '<origin xyz="%.20g %.20g %.20g" rpy="0 0 0"/>' % (com[0], com[1], com[2])
        )
    ...
    def addSTL(self, matrix, stl, color, name, node="visual"):
        ...
        if node == "visual":
            ...
            self.append(
                '<color rgba="%.20g %.20g %.20g 1.0"/>' % (color[0], color[1], color[2])
            )
                ...
                if shapes is None:
                    ...
                    for shape in shapes:
                        ...
                        if shape["type"] == "sphere":
                            self.append(
                                '<sphere radius="%.20g" />' % shape["parameters"]
                            )
                        self.append("</geometry>")
    ...
    def addJoint(
        ...
        self.append(
            ...
            % (
                self.jointMaxEffortFor(name),
                self.jointMaxVelocityFor(name),
                lowerUpperLimits,
            )
        )
            ...
            for entry in entries:
                if shapes is None:
                    ...
                    for shape in shapes:
                        ...
                        if shape["type"] == "sphere":
                            self.append(
                                "<sphere><radius>%.20g</radius></sphere>"
                                % shape["parameters"]
                            )
                        self.append("</geometry>")
