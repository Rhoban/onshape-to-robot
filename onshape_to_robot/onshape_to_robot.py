...
from .message import error, bright, info, warning, dim, success
import sys
...
def main():
    ...
    from .load_robot import config, client, tree, occurrences, getOccurrence, frames
    ...
    if config["outputFormat"] == "urdf":
        ...
        print(
            error(
                f"ERROR: Unknown output format: {config['outputFormat']} (supported are urdf and sdf)"
            )
        )
        exit()
    ...
    def addPart(occurrence, matrix):
        ...
        if part["partId"] == "":
            print(warning(f"WARNING: Part {part['name']} has no partId"))
            return
        ...
        if occurrence["instance"]["configuration"] != "default":
            extra = dim(
                " (configuration: " + occurrence["instance"]["configuration"] + ")"
            )
        symbol = "+"
        ...
        print(success(f"{symbol} Adding part {occurrence['instance']['name']}{extra}"))
        ...
        if config["noDynamics"]:
            ...
            if prefix in config["dynamicsOverride"]:
                ...
                if part["partId"] not in massProperties["bodies"]:
                    print(
                        warning(
                            f"WARNING: part {part['name']} has no dynamics (maybe it is a surface)"
                        )
                    )
                    return
                ...
                if abs(mass) < 1e-9:
                    print(
                        warning(
                            f"WARNING: part {part['name']} has no mass, maybe you should assign a material to it ?"
                        )
                    )
    ...
    def buildRobot(tree, matrix):
        ...
        print(info(f"* Adding part {instance['name']}"))
    ...
    print(bright(f"* Writing {robot.ext.upper()} file"))
    with open(
